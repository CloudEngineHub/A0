...
def inference_fn(args,split='test'):
    ...
    if output_dir:
        output_subdir = os.path.join(   args.image_save_path,'hoi4d_image')
    
    policy = make_policy(args)

    dataset = HOI4DWaypointFrameDataset(split=split,first_frame=args.first_frame)
    num = len(dataset)
    ...
    for idx in tqdm(range(idxx,idxx+num,1)): 
        ...
        objectt = sample['meta']['object']

        action_gt = sample['actions']

        points_gt = action_gt
        # state_indicator = sample['state_indicator']
        ...
        print("points_gt", points_gt.shape, points_gt)
        ...
        if output_dir:
            ...
            draw_arrows_on_image_cv2(image, points, save_path=os.path.join(output_dir,output_subdir,split,f"{split}_id_{idd}.jpg"))
