...
def train(args, logger):
    ...
    if args.precomp_lang_embed:
        ...
        if args.text_encoder == "t5-v1_1-xxl":
        ...
        elif args.text_encoder == "Qwen2.5-7B":
            text_embedder = Qwen2Embedder(from_pretrained=args.pretrained_text_encoder_name_or_path, 
                                    model_max_length=config["dataset"]["tokenizer_max_length"], device=accelerator.device,)
    ...
    train_dataset = VLAConsumerDataset(
        ...
        crop_rotate_aug= args.crop_rotate_aug,
    ...
    # print('*'*50,len(train_dataloader),len(sample_dataloader))
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("a0_diffusion_transformer", config=vars(args))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for batch in train_dataloader:
            with accelerator.accumulate(rdt):
                ...
                with torch.no_grad():
                    batch_size, _, C, H, W = images.shape # B，2,3,384,384
                    image_embeds = vision_encoder(images.reshape(-1, C, H, W)).detach()
                    # 200, 729, 1152

                    # # motion tokens attention
                    if config["common"]["img_history_size"]==2:
                        image_diffs = image_embeds[1::2, :, :] - image_embeds[0::2, :, :]
                        image_embeds[0::2, :, :] = image_diffs
                        # image_embeds = torch.cat([image_embeds, image_diffs], dim=0)

                    image_embeds = image_embeds.reshape((batch_size, -1, vision_encoder.hidden_size))
                    # image_embeds.shape #[100, 1458, 1152])
            ...
            logs = {"mse_w": loss.detach().item(),"mse":loss_dict['mse_loss'].detach().item(),
                    "l1_loss":loss_dict['l1_loss'].detach().item(),"l1_loss_w":loss_dict['l1_loss_weighted'].detach().item(),
